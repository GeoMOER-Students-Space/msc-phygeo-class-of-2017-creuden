...
if (useGRASS){
  ...
  for (j in 1:(length(lasfiles))){
      ...
      result<-link2GI::linkGRASS7(spatial_params = c(ext[1],ext[2],ext[3],ext[4],proj4),resolution = gridsize)
    ...
    slices[[j]] <- try(stack(lapply(heightClassList ,
                                  function(x){raster::raster(tryCatch(rgrass7::readRAST(paste0("class",x[1],x[2])),NODATA=-9999))
                                  }
    )
    ))

    stat[[j]] <- try(stack(lapply(statList ,
                                  function(x){raster::raster(tryCatch(rgrass7::readRAST(paste0(x,"_veg")),NODATA=-9999))
                                  }
    )
    ))
    plot(slices[[j]])
    ...
    #--> FHD using the fun_fhd function  provided in diversityindeces.R
    fhd[[j]]<- fun_fhd(slices[[j]])
    plot(fhd[[j]])
    #--> FHD using the fun_fhd function  provided in diversityindeces.R
    vdr[[j]]<- fun_vdr(stat[[j]][[1]],stat[[j]][[2]])
    plot(vdr[[j]])
    
    # spplot(plot(fhd[[j]]), scales = list(draw = TRUE), 
    #        xlab = "easting", ylab = "northing", 
    #        col.regions = heat.colors(99), 
    #        names.attr=c('original', 'times two'))
    
        
    # spplot(vdr[[j]], scales = list(draw = TRUE), 
    #        xlab = "easting", ylab = "northing", 
    #        col.regions = heat.colors(99), 
    #        names.attr=c('original', 'times two'))
