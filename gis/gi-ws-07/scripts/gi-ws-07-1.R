...
for (j in 1:(length(lasfiles))) {
  ...
  slices[[j]] <- stack(lapply(heightClassList ,
                              function(x){raster::raster(rgrass7::readRAST(paste0("class",x[1],x[2]),NODATA=-9999))
                              }
  )
  )
  
  stat[[j]] <- stack(lapply(statList ,
  ...
  if (plotIt){
    ...
    mapview::mapview(vdr[[j]],
                     ...
                     alpha.regions = 0.3,
                     layer.name="FHD Index")
    plot(vdr[[j]],  col=rev(heat.colors(10)),main="FHD Index")
  }
}
